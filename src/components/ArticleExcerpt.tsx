...
export function ArticleExcerpt({ url, source, fallbackSummary }: Props) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`/api/article-preview?url=${encodeURIComponent(url)}`)
      .then((r) => r.json())
      .then((d: ExcerptData) => {
        if (!ignore) setData(d);
      })
    ...
    return fallbackSummary ? (
      <div className="space-y-3">
        ...
        <p className="text-[10px] text-[var(--text-muted)]/60">
          Preview excerpt provided by the publisher feed. Continue to {source} for the full story.
        </p>
      </div>
    ) : null;
  }
        ...
        <div className="relative">
          ...
          <div className="rounded-xl border border-[var(--border)] bg-[var(--accent)]/5 p-4 text-center space-y-2">
            <p className="text-xs text-[var(--text-muted)]">
              You&apos;ve read a preview of this story. The full article continues at the original publisher.
            </p>
            <a
              href={url}
              target="_blank"
